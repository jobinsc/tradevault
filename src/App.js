  ...
  const removeFromWatchlistHandler = async (symbol) => {
    try {
 ...
 const getHoldingDuration = (entryDate, exitDate) => {
  ...
  // Check if exitDate is truly valid (not empty, not "-", not "undefined")
  const hasValidExit = exitDate && 
                       exitDate !== '-' && 
                       exitDate !== 'undefined' &&
                       String(exitDate).trim() !== '';
  
  const end = hasValidExit ? parseISO(exitDate) : new Date();
  if (isNaN(end.getTime())) return '';
  
  const days = differenceInDays(end, start);
  
  // Safety: negative days = bad data
  if (days < 0) return '';
  
  if (days < 1) return hasValidExit ? 'Same Day' : 'Today';
  ...
  function App() {
    ...
    if (data.entryDate && new Date(data.entryDate) > today) {
      showMsg('❌ Entry date cannot be in the future!');
      return;
    }

    if (data.exitDate && new Date(data.exitDate) > today) {
      showMsg('❌ Exit date cannot be in the future!');
      return;
    }

    if (data.entryDate && data.exitDate && 
        new Date(data.exitDate) < new Date(data.entryDate)) {
      showMsg('❌ Exit date cannot be before entry date!');
      return;
    }
  ...
  function TradeModal({ trade, onSave, onClose }) {
    const [form, setForm] = useState({
      ...
      emotion: '', rating: 0, notes: '', setup: '', tags: [], sector: '',
    ...
    const upd = (k, v) => {
      setForm(p => {
        const updated = { ...p, [k]: v };
        // 🏷️ Auto-detect sector when symbol changes
        if (k === 'symbol' && v) {
          updated.sector = getSectorForSymbol(v);
        }
        return updated;
      });
    };
  ...
  <div style={{ position: 'relative' }}>
    ...
    <div style={{ 
      ...
      <span style={{
        background: getSectorColor(form.sector),
        color: '#fff',
        padding: '4px 12px',
        borderRadius: 12,
        fontSize: 12,
            ...
            <span style={{
              ...
              {stock.exchange}
                ...
                <div className="form-group">
                  ...
                  <input type="date" max={new Date().toISOString().split('T')[0]} value={form.entryDate} onChange={e => upd('entryDate', e.target.value)} required />
                </div>
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input type="date" max={new Date().toISOString().split('T')[0]} value={form.exitDate} onChange={e => upd('exitDate', e.target.value)} />
                </div>
                <div className="form-group">
  ...
  // ============ DETAILED REPORT ============
  function DetailedReport({ trades, capital, stats, filterLabel = 'All Trades' }) {
    ...
    return (
      <div>
        ...
        <div style={{ background: 'linear-gradient(135deg, #1e293b, #0f172a)', borderRadius: 16, padding: 24, marginBottom: 20, border: '1px solid var(--border-color)' }}>
          <h2 style={{ fontSize: 22, fontWeight: 800, marginBottom: 6 }}>📊 Detailed Report — {filterLabel}</h2>
          <p style={{ color: 'var(--text-muted)', fontSize: 13 }}>
